...
module photons_mod
    ...
    ! TODO(anton): Make this readable from configuration or such.
    ! Now it is hardcoded to work with 2s2p resonances in neon.
...
contains
    ...
    subroutine get_xuv_photons_linspace(photons)
        ...
        if(allocated(photons%list_au)) then
            ...
            allocate(photons%list_au(g_num_photons))
        end if

        N = g_num_photons
        photons%size = N
        photons%start_eV = g_start_eV
        photons%end_eV = g_end_eV
        delta_eV = (g_end_eV-g_start_eV)/real(N, kind=dp)
